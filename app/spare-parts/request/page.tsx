...
export type ApiSparePartItem = {
  id?: number; // detail row id (used for update/delete)
  sparepartsrequest_id?: number; // optional, present in some responses
  spare_part: string;
  class_type: string;
  qty: number;
  price: number;
...
export type ApiSparePartsResponse = {
  ...
  address: string;
  city: string;
  viN: string; // keep exact casing from API
  spareparts_type: string;
  state: string; // e.g. "new"
  spare_part: ApiSparePartItem[];
  manager_mobile: string;
  id: number; // review id (separate from sparepartsrequest_id)
  request_status: string; // accept many variants from server
};

// Mapped client model used by UI
export type SparePartRequestUI = {
  id: number; // review id
  sparepartsrequest_id: number; // needed for Accept/Decline API
  date: string;
  serviceLocation: string; // "branch_name, address, city"
  branchName: string;
  address: string;
  city: string;
  vinOrPlate: string; // viN
  carPart: string; // spareparts_type
  state: string;
  spareParts: ApiSparePartItem[];
  managerMobile: string;
  status: TabStatus; // mapped from request_status
};

// Map many possible server strings to our three tabs
const statusMap: Record<string, TabStatus> = {
  // accepted offers (many variants)
  ...
  // accepted requests (many variants)
...
const tabItems = [
  ...
  },
  {
    ...
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path d="M3 20C5.33579 17.5226 8.50702 16 12 16C15.493 16 18.6642 17.5226 21 20M16.5 7.5C16.5 9.98528 14.4853 12 12 12C9.51472 12 7.5 9.98528 7.5 7.5C7.5 5.01472 9.51472 3 12 3C14.4853 3 16.5 5.01472 16.5 7.5Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    ),
  },
];
...
export default function SparePartsRequestPage() {
  ...
  // This holds the SERVER data mapped to our UI model
  const [sparePartRequests, setSparePartRequests] = useState<SparePartRequestUI[]>([]);

  // 1) Ensure user_id in localStorage, 2) fetch data by user, 3) map to UI
  useEffect(() => {
    // Temporary: set user_id = 1 if not already present
    const existing = localStorage.getItem("user_id");
    if (!existing) localStorage.setItem("user_id", "1");
  ...
  // Filter by active tab + selected filters (for table rows)
  ...
  // ✅ Counts now re-evaluate when selectors change (so the badges update live)
  ...
  // Local-only updater that the table can call after a successful server update
  ...
  return (
    <div className="bg-gray-50 pb-20">
      ...
      <section className="max-w-[1120px] mx-auto px-4 mb-8">
        <h2 className="text-[#3F72AF] text-2xl md:text-[32px] font-semibold">Spare part request</h2>
        <p className="text-[#ADB5BD] text-xl md:text[20px] mt-2">See your scheduled services from your calendar.</p>

        {/* Filters */}
        <div>
          <label className="block mt-6 text-sm text-[#495057]">Selected spare part</label>
          <div className="mt-3 flex gap-3 flex-wrap">
            ...
            <div className="relative">
              ...
              >
                {uniqueVins.map((v) => (
                  <option key={v} value={v}>{v === "All" ? "VIN / Plate number (All)" : v}</option>
                ))}
              </select>
              <div className="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      <section className="border-b border-gray-200 mb-8">
        <div className="flex gap-3 sm:gap-10 max-w-[1120px] mx-auto px-4">
          {(["Accepted offers", "Accepted requests", "Pending"] as TabStatus[]).map((tab) => {
            const isActive = activeTab === tab;
            const borderColor = tab === "Accepted offers" ? "border-blue-400" : tab === "Accepted requests" ? "border-green-400" : "border-yellow-400";
            return (
              <button
                key={tab}
                className={`px-0 py-4 font-medium flex gap-1 sm:gap-3 items-center ${isActive ? `text-gray-700 border-b-2 ${borderColor}` : "text-gray-300 hover:text-gray-700"}`}
                onClick={() => setActiveTab(tab)}
              >
                <img
                  src={tab === "Accepted offers" ? "/icons/clock-fast-forward.svg" : tab === "Accepted requests" ? "/icons/check-circle-broken.svg" : "/icons/clock-refresh.svg"}
                  width={24}
                  alt={`${tab} icon`}
                />
                <p>{tab}</p>
                <span className="px-3 py-[2px] border border-[#E9ECEF] rounded-3xl">{tabCounts[tab]}</span>
              </button>
            );
          })}
        </div>
