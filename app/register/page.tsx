	...
	return (
		<div className={'bg-light-gray'}>
			<Container className={'flex'}>
							...
							{choices.map((choice) => {
								...
								return (
									<div
										...
										key={choice.text}
										className={cn(
											'cursor-pointer duration-300 ease-linear items-center bg-white flex flex-col gap-y-8 rounded-3xl py-16 px-14',
											choice.id === selectedChoice && 'border border-steel-blue/20 shadow-[0px_24px_32px_0px_#DBE2EF]'
										)}
									>
										<IconComponent />
				...
				{showForm && selectedChoice === 0 && (
					<UserRegistration
						openPopup={() => setIsPopupOpen(true)}
						closeFormAndGoBack={closeFormAndComeBack}
					/>
				)}

				{/* ✅ FIX: Wrap ServiceRegistration with Elements so useStripe() works in step-3 */}
				...
				{/* Keep StoreRegistration wrapped (already correct) */}
				{showForm && selectedChoice === 2 && (
					<Elements stripe={stripePromise}>
						<StoreRegistrationFull
							openPopup={() => setIsPopupOpen(true)}
							closeFormAndGoBack={closeFormAndComeBack}
						/>
					</Elements>
