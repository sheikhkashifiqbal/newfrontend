		...
		<nav className={cn('hidden sm:block', navClassname)}>
			<ul className={cn('flex items-center gap-x-10', ulClassname)}>
				{navLinks.map((nLink) => {
					...
					return (
						<Link href={nLink.path} className={cn(`cursor-pointer text-charcoal80 text-base font-medium`, liClassname, isActive && 'font-semibold text-charcoal')} key={nLink.id}>
							{nLink.text}
						</Link>
					)
				})}
...
export function HeaderButtons({ divClassname }: { divClassname?: string }) {
	return (
		<div className={cn('hidden sm:flex items-center gap-x-6', divClassname)}>
			<LoginButton />
			<Button asChild={true} className={'flex justify-center items-center bg-steel-blue/10 border-[1px] border-steel-blue/20 rounded-[32px] py-2 px-6 hover:bg-steel-blue/25'}>
