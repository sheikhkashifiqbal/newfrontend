...
interface IActionButtons {
  onReserveClick?: () => void;
  // New props to support Make unavailable / Make available logic
  isUnavailable?: boolean;
  onToggleUnavailable?: () => void;
}

export default function ActionButtons({ onReserveClick, isUnavailable = false, onToggleUnavailable }: IActionButtons) {
  return (
    <div className={'hidden h-[6.75rem] duration-700 w-full group-hover:flex gap-2'}>
      <div
        onClick={onReserveClick}
        className={
          'cursor-pointer flex basis-1/2 items-center justify-center flex-col gap-2 bg-steel-blue/10 rounded-2xl p-4 h-full'
        }
      >
      ...
      <div
        ...
        className={
          'cursor-pointer flex basis-1/2 items-center justify-center flex-col gap-2 bg-danger-red/10 rounded-2xl p-4 h-full'
        }
      >
